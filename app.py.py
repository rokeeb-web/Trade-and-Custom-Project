...
# Sidebar file uploader for flexibility
...
if not df.empty:
    ...
    total_imports = df["Import Value"].sum()
    total_tax = df["Tax Revenue"].sum()
    unique_hs_codes = df["HS Code"].nunique()
    unique_countries_origin = df["Country of Origin"].nunique()

    col1, col2, col3, col4 = st.columns(4)
    col1.metric("Total Imports (Value)", f"${total_imports:,.0f}")
    ...
    st.subheader("Top HS Codes by Import Value")
    top_hs = df.groupby("HS Code")["Import Value"].sum().sort_values(ascending=False).head(10)
    fig, ax = plt.subplots(figsize=(8, 5))
    sns.barplot(x=top_hs.values, y=top_hs.index, ax=ax)
    ax.set_xlabel("Import Value")
    ax.set_ylabel("HS Code")
    st.pyplot(fig)

    # 2. Top Countries of Origin
    st.subheader("Top Countries of Origin")
    top_origin = df.groupby("Country of Origin")["Import Value"].sum().sort_values(ascending=False).head(10)
    fig, ax = plt.subplots(figsize=(8, 5))
    ...
    st.subheader("Top Countries of Supply")
    top_supply = df.groupby("Country of Supply")["Import Value"].sum().sort_values(ascending=False).head(10)
    fig, ax = plt.subplots(figsize=(8, 5))
    ...
    top_tax = df.groupby("HS Code")["Tax Revenue"].sum().sort_values(ascending=False).head(10)
    fig, ax = plt.subplots(figsize=(8, 5))
    ...
    st.subheader("Top Importers by Import Value")
    top_importers = df.groupby("Importer")["Import Value"].sum().sort_values(ascending=False).head(10)
    fig, ax = plt.subplots(figsize=(8, 5))
